 ...
 */
export function extractPublicKey(credential: PublicKeyCredential): ArrayBuffer | null {
...
import { createPasskey, getPasskey, storeCredentialId, getStoredCredentialId } from '@/utils/passkeys';
  ...
  const handleCreatePasskey = async () => {
    try {
      ...
      if (credential.rawId) {
        ...
        setCredentialId(btoa(String.fromCharCode(...new Uint8Array(credential.rawId))));
